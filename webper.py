...
if __name__ == "__main__":
    ...
    def set_overwrite():
        ...
        while overwrite_ans.lower().strip() not in ('y', 'n', 'yes', 'no'):
    ...
    def process_file(name, quality_scale=0):
        ...
        def get_quality(base):
            ...
            if lossless:
                ...
                colors = len({image.getpixel((x,y)) for x in range(0,image.size[0],2) for y in range(0,image.size[1],2)})
                resolution = image.size[0]*image.size[1]

                return int(base
                    + (min(5, get_positive((51200  - size)/9000)))            # 0-5% increase for sizes <50KB
                    + (min(25, get_positive((256000 - colors)/2000)))         # 0-25% increase for <256K colors
                    + (min(10, get_positive((1572864 - resolution)/130000)))) # 0-10% increase for resolutions <1.5 MP
        ...
        mock_filename = f"{file_tuple[0]}.{out_format}"

        if os.path.isfile(mock_filename) and not overwrite:
            # Skip if file with input name and output format already exists
            log(f"\"{mock_filename}\": File already exists, skipping...")
            return

        image = Image.open(name)
        quality = get_quality((55 if unwebp else 45) + quality_scale)

        image.save(fp=f"{file_tuple[0]}.{out_format}", format=out_format, lossless=lossless, quality=quality, method=6)

        diff = get_positive((size - os.path.getsize(f"{file_tuple[0]}.{out_format}"))/1024)
        total_diff += diff

        log(f"\"{name}\" output quality: {quality} ({'' if lossless else 'not '}lossless)")
        if unwebp:
